...
$(function(){
	ue.ready(function() {
		ue.execCommand('insertHtml', '${bean.CONTENT}');
	});
  });
	function getContext(){
		var arr = [];
...
<body>
	...
	<div class="divclass">
		...
		<span><input id="PUBLISH_TIME" name="PUBLISH_TIME" value="${bean.PUBLISH_TIME}" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'2008-03-08 11:30:00'})" type="text" placeholder="" autocomplete="off"></span>
	</div>
	<div class="divclass">
		<span class="h_class_name">上线开始时间</span>
		<span><input id="ONLINE_START_TIME" name="ONLINE_START_TIME" value="${bean.ONLINE_START_TIME}"  onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'2008-03-08 11:30:00'})"  type="text" placeholder="" autocomplete="off"></span>
	</div>
	<div class="divclass">
		<span class="h_class_name">上线结束时间</span>
		<span><input id="ONLINE_END_TIME" name="ONLINE_END_TIME" value="${bean.ONLINE_END_TIME}"  onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'2008-03-08 11:30:00'})" type="text" placeholder="" autocomplete="off"></span>
	</div>
	<div class="divclass">
...
/* 				if($.trim(IN_USE_FLAG).length==0){
				...
				if(confirm('确实要提交吗？')){
					$.ajax({
						type : 'post',
						url : 'update.htm',
						data:{'ID':'${param.ID}','NAME':NAME,'REMARK':REMARK,'IN_USE_FLAG':IN_USE_FLAG,'PUBLISH_TIME':PUBLISH_TIME,'RICH_CONTENT_ID':RICH_CONTENT_ID,'ONLINE_START_TIME':ONLINE_START_TIME,'ONLINE_END_TIME':ONLINE_END_TIME,
							'SHOW_ORDER':SHOW_ORDER,'NEWSURL':NEWSURL,
