        ...
        function sendCode() {
          ...
          if(!tel){
		      ...
		      $("#code").find(".card-txt").val("")
         	  loading("start");
	          $.ajax({                        //发送验证码
	          url: getRootPath() + "/appMng/ManageC/sendRegistCode.htm",
	          data: {"tel":tel},
	          dataType: "json",
	          type: "post",
	          success: function (data) {
                if(-1==data.timeout){
              		// session已经过期了
              		window.location=getRootPath() + "/appMng/ManageC/userIn.htm";
              		return;
                }
	          	loading("stop");
		        if(data.code==1){
        ...
        function submitInfo() {
            ...
            if(!tel){
                ...
                TipShow('请输入验证码',1000);
   				...
   				$.ajax({
   					//zanwu
   					url : getRootPath() + "/appMng/ManageC/userLogin.htm",
   					data : {
   						"tel" : tel,
   						"code" : code
   					},
   					...
   					success : function(data) {
   						loading("stop")
   						if (data.code==1) {
                            window.location=getRootPath() + "/weixinMng/ManageC/userIn.htm?enterType=app";
   						} else {
   							alerw(data.msg);
   						}
   					}
...
<div class="zoomer">
    <ul class="flex index_banner_top">
        <li class="index_banner_top_personDiv" onclick="personInfo()">
            <div class="flex">
                <div>
                    <img src="${contentPath }/jsp/weixinMng/mallMng/img/icon.png" alt="" class="icon11_img">
                </div>
                <span class="flex-1 personInfo2">
                        <span class="index_banner_top_personDivName">登录</span>
                    </span>
            </div>
            <div class="flex index_rangeDiv">
                ...
                <div class="flex-1">
                    <div class="index_range_amount">0/99</div>
                    <div class="index_range">
                        <div class="index_rangeV" style="width: 0%"></div>
                    </div>
                </div>
        ...
        <ul class="flex" style="padding-top: 20px;">
            ...
            <li class="flex-1 jmt_center" onclick="gotomyGame()">
                ...
                <div class="icon_bg">
                    ...
                    <div class="indexIcon_text2"><a>0</a>收藏</div>
                </div>
            </li>
            <li class="flex-1 jmt_center" onclick="gotorank()">
                ...
                <div class="icon_bg">
                    ...
                    <div class="indexIcon_text2"><a>0</a>入榜</div>
                </div>
            </li>
        </ul>
        <div class="gameRank">
