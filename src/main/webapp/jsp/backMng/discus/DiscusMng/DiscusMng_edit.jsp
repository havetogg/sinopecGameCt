...
<%-- 		<span><input id="CONTENT" name="CONTENT" value="${bean.CONTENT}" type="text" placeholder="" autocomplete="off"></span> --%>
	<textarea id="CONTENT" name="CONTENT" style="width: 500px; height: 180px;margin: 0 auto;">${bean.CONTENT}</textarea>
	</div>
	 <div>
            <span class="h_class_name">上传图片</span>
            <label for="IMAGE_UPLOAD"><img id="uploadImg" src="${contentPath }/${bean.CUTTED_PATH}"  style="width: 100px;height: 100px;" alt=""></label>
     		<input type="hidden" id="IMAGE_URL" value="${bean.ORIGINAL_URL}@@${bean.CUTTED_PATH}"/>
...
/* 				if($.trim(IN_USE_FLAG).length==0){
				...
				if(confirm('确实要提交吗？')){
					$.ajax({
						type : 'post',
						url : 'update.htm',
						data:{'ID':'${param.ID}','NAME':NAME,'IN_USE_FLAG':IN_USE_FLAG,'PUBLISH_TIME':PUBLISH_TIME,'CONTENT':CONTENT,'IMAGE_URL':IMAGE_URL,'ONLINE_START_TIME':ONLINE_START_TIME,'ONLINE_END_TIME':ONLINE_END_TIME,'SHOW_ORDER':SHOW_ORDER},
						dataType : 'json',
