			...
			$("#save").click(function(){
				...
				if(confirm('确实要提交吗？')){
					$.ajax({
						type : 'post',
						url : 'update.htm',
						data:{'ID':'${param.ID}','NAME':NAME,'IN_USE_FLAG':IN_USE_FLAG,'PUBLISH_TIME':PUBLISH_TIME,'CONTENT':CONTENT,'ORIGINAL_URL':ORIGINAL_URL,'CUTTED_PATH':CUTTED_PATH,'REPLY_NUM':REPLY_NUM,'PRAISE_NUM':PRAISE_NUM,'CONCERN_NUM':CONCERN_NUM,'ONLINE_START_TIME':ONLINE_START_TIME,'ONLINE_END_TIME':ONLINE_END_TIME,'CREATE_TIME':CREATE_TIME,'LAST_MODIFY_TIME':LAST_MODIFY_TIME,'SHOW_ORDER':SHOW_ORDER,'DELETE_FLAG':DELETE_FLAG},
						dataType : 'json',
