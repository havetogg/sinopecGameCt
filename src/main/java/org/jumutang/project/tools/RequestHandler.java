...
public class RequestHandler {
	...
	private static final Logger _LOGGER = Logger.getLogger(RequestHandler.class);
	/**
	 * 初始构造函数。
	 * 
	 * @return
...
//		System.out.println("UrlEncode后 packageValue=" + packageValue);
	...
	public static String createSign(SortedMap<String, String> packageParams) throws UnsupportedEncodingException {
		...
		_LOGGER.info("strec:"+strec);
//		String sign = MD5Util.MD5Encode(UrlEncode(strec), "UTF-8");
		String sign = MD5Util.md5Encodenew(UrlEncode(strec));
		_LOGGER.info("sign:"+sign);
		return sign;
	 ...
	 * 创建md5摘要,规则是:按参数名称a-z排序,遇到空值的参数不参加签名。
	 */
	public String createPaySign(SortedMap<String, String> packageParams) {
		StringBuffer sb = new StringBuffer();
		Set es = packageParams.entrySet();
		Iterator it = es.iterator();
		while (it.hasNext()) {
